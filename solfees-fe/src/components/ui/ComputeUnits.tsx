...
const percentFormatter = new Intl.NumberFormat("unknown", {
  style: "percent",
  maximumFractionDigits: 2,
  minimumFractionDigits: 2,
  ...
  return (
    <div className="px-3 min-w-0">
      {items.map((elt, idx) => (
        ...
        >
          <Text font="mono">{nFormatter(elt.amount, 2, true)}</Text>
          <Text font="mono">({percentFormatter.format(elt.percent).replace(/\s/g, "")})</Text>
        </div>
      ))}
    </div>
  );
