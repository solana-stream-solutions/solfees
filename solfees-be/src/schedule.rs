...
pub struct LeadersScheduleSolfees {
    leaders: Vec<String>,
    indices: LeaderScheduleIndices,
}

impl From<LeadersSchedule> for LeadersScheduleSolfees {
    fn from(schedule: LeadersSchedule) -> Self {
        Self {
            leaders: schedule
                .leaders
                .into_iter()
                .map(|pk| pk.to_string())
                .collect(),
            indices: LeaderScheduleIndices(schedule.indices),
        }
    }
}
